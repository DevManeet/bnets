...
| path         | Persistence path for QdrantLocal. Eg. `local_data/bnet/qdrant`|
...
| Field        | Description |
|--------------|-------------|
| uri     | Default is set to "local_data/bnet/milvus/milvus_local.db" as a local file; you can also set up a more performant Milvus server on docker or k8s e.g.http://localhost:19530, as your uri; To use Zilliz Cloud, adjust the uri and token to Endpoint and Api key in Zilliz Cloud.|
...
To obtain a local setup (disk-based database) without running a Milvus server, configure the uri value in settings.yaml, to store in local_data/bnet/milvus/milvus_local.db.
...
| **schema_name** | The database schema to use. Default is `bnet`       |
...
```
postgres=# \d bnet.data_embeddings
                                      Table "bnet.data_embeddings"
  Column   |       Type        | Collation | Nullable |                         Default
-----------+-------------------+-----------+----------+---------------------------------------------------------
 id        | bigint            |           | not null | nextval('bnet.data_embeddings_id_seq'::regclass)
