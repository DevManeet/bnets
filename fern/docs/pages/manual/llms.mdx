...
Make sure you have followed the _Local LLM requirements_ section before moving on.
...
```bash
PGPT_PROFILES=local poetry run python -m bnet
```

When the server is started it will print a log _Application startup complete_.
...
these at the `llm_component.py` file under the `bnet/components/llm/llm_component.py`.
...
openai:
  ...
  model:
    <openai_model_to_use> # Optional model to use. Default is "gpt-3.5-turbo"
...
`PGPT_PROFILES=openai poetry run python -m bnet`
...
`PGPT_PROFILES=azopenai poetry run python -m bnet`
...
`PGPT_PROFILES=sagemaker poetry run python -m bnet`
...
ollama:
  model:
    <ollama_model_to_use> # Required Model to use.
...
`PGPT_PROFILES=ollama poetry run python -m bnet`
...
To deploy Ollama and pull models using IPEX-LLM, please refer to [this guide](https://ipex-llm.readthedocs.io/en/latest/doc/LLM/Quickstart/ollama_quickstart.html). Then, follow the same steps outlined in the [Using Ollama](#using-ollama) section to create a `settings-ollama.yaml` profile and run the bnet server.
...
`PGPT_PROFILES=gemini poetry run python -m bnet`
